Adding or Updgrading SIMP Packages.
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
and later. 
They assume the Puppet environment you are updating is named ``test``, and that you execute these
steps as ``root``:

Beore Upgrading/Updating a :ref:`Local deployment
